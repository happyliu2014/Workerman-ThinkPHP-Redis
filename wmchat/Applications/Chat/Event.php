...
class Event
{
   ...
   public static function onMessage($client_id, $message)
   {
        ...
        switch($message_data['type'])
        {
            // 客户端回应服务端的心跳
            case 'pong':
				echo RedisDb::instance('redis')->get('t1');
                return;
            ...
            case 'say':
                ...
                if($message_data['to_client_id'] != 'all')
                {
                    $new_message = array(
                        ...
                        'content'=>"<strong>对你说: </strong>".nl2br(htmlspecialchars($message_data['content'])),
                        'time'=>date('Y-m-d H:i:s'),
                    );
                    Gateway::sendToClient($message_data['to_client_id'], json_encode($new_message));
                    $new_message['content'] = "<strong>你对".htmlspecialchars($message_data['to_client_name'])."说: </strong>".nl2br(htmlspecialchars($message_data['content']));
